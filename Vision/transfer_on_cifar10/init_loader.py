...
def init(conf=None , use_consistency_loss:bool = False, next_parameters:dict=None,model = None, block_to_friz=0):
	...
	wandb.init(project='<Fashion_MNIST_Transfer>', entity='idori2', mode="online",
			   tags=[str(conf['dataset_percentege']),str(conf['layers_to_freeze']),conf['dataset'], conf['model_conf']['model_name']])
	...
	transform = transforms.Compose([transforms.Grayscale(num_output_channels=1),
									transforms.Pad((conf['padded_im_size'] - conf['im_size']) // 2),
									transforms.ToTensor(),
									transforms.Normalize(tuple(conf['dataset_mean']), tuple(conf['dataset_std']))])

	
	train_dataset = datasets.CIFAR10("Vision/data/CIFAR10",train=True,download=True,transform = transform)
	print('len train_dataset= ',int(50000*(conf['dataset_percentege']/100)))
	train_dataset = torch.utils.data.Subset(train_dataset,list(range(int(50000*(conf['dataset_percentege']/100)))))
	g = torch.Generator()
	...
	for child in model.children():
		...
		# print(child)
		# print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
		if ct <= (4 + conf['layers_to_freeze']) and conf['layers_to_freeze'] != 0  :
